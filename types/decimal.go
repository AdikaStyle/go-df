package types

import (
	"fmt"
	"github.com/cstockton/go-conv"
)

type DecimalType float64

func (i DecimalType) String() string {
	str, err := conv.String(float64(i))
	if err != nil {
		panic(err)
	}
	return str
}

func (i DecimalType) Equals(other Type) bool {
	if i.Kind() == other.Kind() {
		return i == other
	}
	return false
}

func (i DecimalType) Compare(other Type) TypeComparision {
	if i.Kind() != other.Kind() {
		panic(fmt.Sprintf(
			"couldn't compare between different kind of types where left is: %s(%v) and right is: %s(%v)",
			i.Kind(), i, other.Kind(), other),
		)
	}

	otherVal, err := conv.Float64(other)
	PanicOnError(err)
	if float64(i) == otherVal {
		return Equals
	} else if float64(i) > otherVal {
		return LeftIsBigger
	} else {
		return RightIsBigger
	}
}

func (i DecimalType) Cast(toPtr interface{}) {
	if err := conv.Infer(toPtr, i); err != nil {
		panic(err)
	}
}

func (i DecimalType) Kind() TypeKind {
	return Decimal
}
